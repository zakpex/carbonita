create or replace PACKAGE carbonita_plugin_pkg as 
...
/
create or replace PACKAGE BODY CARBONITA_plugin_pkg AS
...
-----------------------------------------------------------------------------------------------------------------
 ...
 -- plugin attributes
  ...
  -- other vars
        ...
        l_js_data_json_query   VARCHAR2(4000)   := p_dynamic_action.attribute_01; --TOCHECK TODO check how dangerous  than apex_application.g_x05;
        ...
        l_plg_report_type       VARCHAR2(250)   := lower(apex_application.g_x02);-- TO check context
        l_plg_report_name       VARCHAR2(250)   := apex_application.g_x03;-- TO check context
        
        l_plg_ptres_names   VARCHAR2(250) := apex_application.g_x04;
        l_plg_ptres_values  VARCHAR2(250) := apex_application.g_x05;
        ------------------------------------------------------------
        
        
       -- l_plg_template_item  VARCHAR2(250)      := apex_application.g_x04;-- TO check context
      --  l_plg_reporttype_item  VARCHAR2(250)    := apex_application.g_x05;-- TO check context
    ...
    BEGIN
        BEGIN -- prepare  template
            ...
            EXCEPTION WHEN OTHERS THEN  
                begin
                message_error(l_plg_template_filename,sqlerrm);           
        ...
        begin -- prepare data    
                ...
                l_data_clob := carbonita_pkg.get_data_clob(
                                ...
                                p_parameter_holders => l_plg_ptres_names,
                                p_parameter_values  => l_plg_ptres_values,
                                p_separator         => ','
                        );
            EXCEPTION WHEN OTHERS THEN 
                begin 
                    message_error(l_js_data_json_query,sqlerrm); 
  ...
  -------------------------------------------------------------------------------- 
    FUNCTION plugin_process (
        ...
        l_plg_ptres_values  VARCHAR2(4000)        ;
  ...
  l_chunk  NUMBER := 3000;
    BEGIN
        ...
        l_plg_ptres_names              := p_process.attribute_05 ;
        l_plg_ptres_values             := p_process.attribute_06 ;  
...
END CARBONITA_plugin_pkg;
/
create or replace PACKAGE CARBONITA_PKG AS 

    function get_data_clob (-- without paremeter
        p_query_json in varchar2 default q'#select JSON_ARRAYAGG(json_object(ename,job)) val from emp#'
                            ) return clob;
    
     FUNCTION get_data_clob ( -- with parameter
...
END CARBONITA_PKG;
/
create or replace PACKAGE BODY carbonita_pkg AS
    ...
    BEGIN
        begin -- loop from split parameter varchar2 TODO TOCHECK  !!!! 
                    if (1=1) then 
                        ...
                        for c_var in (select column_value from apex_string.split(p_parameter_holders,p_separator) ) loop
                            i:= i+1; L_PLACEHOLDERS.extend();
                            L_PLACEHOLDERS(i) := c_var.column_value;
                        end loop;
                        i:= 0;   L_VALUES := apex_t_varchar2();
                        for c_var in (select column_value from apex_string.split(p_parameter_values,p_separator) ) loop
        ...
        RETURN l_return_clob;
    END get_data_clob; -- with parameters
